...
import { useAppStore, EnrichmentMode, PRO_ENRICHMENT_MODES } from '@/store/appStore';
...
export default function Home() {
  ...
  const { user, usage, initialized, initialize, logout } = useAuthStore();
            ...
            <div className="relative">
              ...
              {showModeDropdown && (
                <>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 glass-card rounded-2xl overflow-hidden shadow-elevated-lg z-20 animate-fadeIn max-h-80 overflow-y-auto">
                    {/* Free Modes */}
                    ...
                    {/* Pro Modes Divider */}
                    <div className="px-4 py-2 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-t border-b border-amber-500/20">
                      <div className="flex items-center gap-2">
                        <Crown className="w-3.5 h-3.5 text-amber-400" />
                        <span className="text-xs font-semibold text-amber-400">PRO FEATURES</span>
                      </div>
                    </div>
                    ...
                    {PRO_ENRICHMENT_MODES.map((mode) => {
                      ...
                      return (
                        ...
                        >
                          <span className="font-medium text-sm">{getEnrichmentModeLabel(mode)}</span>
                          {settings.enrichmentMode === mode && isPro ? (
                            <span className="w-5 h-5 bg-primary-500 rounded-full flex items-center justify-center">
                              <span className="text-white text-xs">✓</span>
                            </span>
                          ) : !isPro ? (
